...
public interface SpringDataLendingRepository extends LendingRepository, LendingRepoCustom, CrudRepository<Lending, Long> {
    ...
    @Query("SELECT l " +
            "FROM Lending l " +
            "JOIN BookSync b ON l.book.pk = b.pk " +
    ...
    @Query(value =
            "SELECT AVG(DATEDIFF(day, l.start_date, l.returned_date)) " +
                    "FROM Lending l " +
                    "JOIN BOOK_SYNC b ON l.BOOK_PK = b.PK " +
                    "WHERE b.ISBN = :isbn"
            , nativeQuery = true)
...
class LendingRepoCustomImpl implements LendingRepoCustom {
    ...
    public List<Lending> searchLendings(Page page, String readerNumber, String isbn, Boolean returned, LocalDate startDate, LocalDate endDate){
        ...
        final Join<Lending, BookSync> bookJoin = lendingRoot.join("book");
    ...
    public List<GenreLendingsPerMonthDTO> getLendingsPerMonthLastYearByGenre() {
        ...
        String jpql = "SELECT new pt.psoft.g1.psoftg1.genremanagement.services.GenreLendingsPerMonthDTO(" +
                "YEAR(l.startDate), MONTH(l.startDate), b.genre, COUNT(l)) " +
                "FROM Lending l JOIN l.book b " +
                "WHERE l.startDate >= :startDate " +
                "GROUP BY YEAR(l.startDate), MONTH(l.startDate), b.genre " +
                "ORDER BY YEAR(l.startDate), MONTH(l.startDate), b.genre";

        return em.createQuery(jpql, GenreLendingsPerMonthDTO.class)
    ...
    public List<GenreLendingsDTO> getAverageLendingsInMonth(LocalDate month, Page page) {
        ...
        String jpql = "SELECT new pt.psoft.g1.psoftg1.genremanagement.services.GenreLendingsDTO(" +
                "b.genre, COUNT(l), AVG(CASE WHEN l.returnedDate IS NOT NULL THEN 1 ELSE 0 END)) " +
                "FROM Lending l JOIN l.book b " +
                "WHERE l.startDate BETWEEN :startDate AND :endDate " +
                "GROUP BY b.genre " +
                "ORDER BY COUNT(l) DESC";

        return em.createQuery(jpql, GenreLendingsDTO.class)
